...
public class Cube {
    ...
    public float getUnsolvedGroup1Pieces() {
        ...
        for (char[] corner : corners) {
            if (corner[0] == 'Y' || corner[0] == 'W')
                count++;
        }
        char[][] edges = { FL, FR, BL, BR };
        for (char[] edge : edges) {
            if (edge[0] == 'G') {
                if (edge[1] == 'O' || edge[1] == 'R')
                    count++;
            } else if (edge[0] == 'B') {
                if (edge[1] == 'O' || edge[1] == 'R')
                    count++;
    ...
    public float getUnsolvedGroup2Pieces() {
        ...
        if (DBR[1] == 'G' || DBR[1] == 'B')
            count++;
        // 8 edges
        if (green[1] == 'G' || green[1] == 'B')
            count++;
        if (orange[1] == 'O' || orange[1] == 'R')
            count++;
        if (orange[7] == 'O' || orange[7] == 'R')
            count++;
        if (green[7] == 'G' || green[7] == 'B')
            count++;
        if (blue[1] == 'G' || blue[1] == 'B')
            count++;
        if (red[1] == 'O' || red[1] == 'R')
            count++;
        if (red[7] == 'O' || red[7] == 'R')
            count++;
        if (blue[7] == 'G' || blue[7] == 'B')
            count++;
        return 16 - count;
    }
    ...
    // String[] moves = {"U", "U'", "U2", "F", "F'", "F2", "B", "B'", "B2", "R",
    // "R'", "R2", "L", "L'", "L2", "D", "D'", "D2",};
    //
    // for (String move : moves) {
    // Cube tempCube = new Cube(new char[][]{green, blue, white, yellow, orange,
    // red}, depthLevel + 1, move);
    ...
    // case "D'" -> DPrimeCube;
    // case "D2" -> D2Cube;
    // default -> throw new IllegalArgumentException("Invalid childName: " +
    // childName);
    // };
    // }
